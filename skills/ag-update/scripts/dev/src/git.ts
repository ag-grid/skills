...
export function gitRepoRoot(cwd: string): string {
    try {
        ...
        throw new ExitWithError('not in a Git repo or Git is not installed', [
            ...
            'To run the script on files not tracked by Git, create a temporary Git repo in an appropriate parent ' +
                'directory and commit the project source files to it. Use .gitignore files to declare which files are ' +
                'not source code files, including node_modules files and build output folders. If there is an ' +
                'alternative VCS in use, use its ignore files or other relavent project configuration to determine ' +
                'the apporpriate files to ignore.',
            "IMPORTANT: creating this temporary Git repo will affect the user's workspace, ask them for permission " +
                'before going ahead and creating a temporary Git repo.',
        ]);
    }
}
