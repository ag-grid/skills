...
test('report groups changes by product and version transition', () => {
    const report = renderReport(
        gridAndChartsProject([
            ...
            {
                product: 'grid',
                change: requirement({ version: '34.0.0', description: 'Pass a params object instead.' }),
                occurrences: [],
            },
            {
                product: 'charts',
                change: behaviourChange({ version: '11.0.0', title: 'legends now paginate by default' }),
                occurrences: [],
            },
        ]),
        changelogs
...
test('requirement/removal/dependency changes appear under Required changes; behaviour/style under Optional changes', () => {
    const report = renderReport(
        gridAndChartsProject(
            [
                ...
                behaviourChange(),
                styleChange(),
            ].map((change) => ({ product: 'grid' as const, change, occurrences: [] }))
        ),
        changelogs
    );
    const requiredSection = report.slice(report.indexOf('# Required changes'), report.indexOf('# Optional changes'));
    ...
    expect(optionalSection).toContain(`#### DECISION: ${behaviourChange().title}`);
