...
- From v13, `axes` is a dictionary keyed `x`/`y` (cartesian) or `angle`/`radius` (polar) — `axes: { x: { type: 'category' }, y: { type: 'number' } }` — not the old array form. Docs: `axes-configuration`, `axes-types`
