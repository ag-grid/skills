...
export async function runTest(def: TestDefinition): Promise<RunResult> {
  ...
  const skillSrc = def.skill
    ? join(REPO, def.skill)
    : join(REPO, "tests/harness/cases", def.name);
  const skillName = basename(skillSrc);
  cpSync(skillSrc, join(workDir, ".claude/skills", skillName), {
    recursive: true,
