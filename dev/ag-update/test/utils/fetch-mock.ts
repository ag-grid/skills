...
/** The genuine global fetch, captured before it is replaced below; the one test that exercises
 *  real http (against a local http.createServer) restores it for its own file. */
...
globalThis.fetch = async (input: string | URL | Request): Promise<Response> => {
  ...
  if (!route) {
    throw new Error(
      `no mock registered for fetch of ${url} — register it with mockHttpResponse/mockHttpFailure`,
    );
  }
  return route();
};
...
export function mockHttpResponse(
  ...
  routes.set(url, () =>
    Promise.resolve(
      typeof response === "string" ? new Response(response) : response,
    ),
  );
}
...
/** Serves one outcome per successive fetch of `url`, for testing retries: a string yields a 200
 *  text response, `'FAIL'` rejects like a network failure. The last outcome repeats once the
 *  sequence is exhausted. */
export function mockHttpSequence(
  url: string,
  outcomes: (string | "FAIL")[],
): void {
  let call = 0;
  routes.set(url, () => {
    ...
    return outcome === "FAIL"
      ...
      : Promise.resolve(new Response(outcome));
  });
}
