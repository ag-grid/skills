...
export function changelog(
  overrides: Partial<CompiledChangelog> = {},
): CompiledChangelog {
  return {
...
export function transition(
  overrides: Partial<CompiledTransition> = {},
): CompiledTransition {
  return {
    type: "transition",
    id: null,
...
export function dependencyChange(
  overrides: Partial<CompiledDependencyChange> = {},
): CompiledDependencyChange {
  return {
    type: "dependency",
    version: "33.0.0",
...
export function mitigation(
  content: string,
  frameworks: CompiledMitigation["frameworks"] = [
    "react",
    "angular",
    "vue",
    "javascript",
...
export function serveChangelogs(
  ...
  for (const [product, log] of Object.entries(changelogs)) {
    mockHttpResponse(
      changelogUrl(prefix, product as Product),
      JSON.stringify(log),
    );
  }
}
...
export function writeChangelogsToDisk(
  changelogs: Partial<Record<Product, CompiledChangelog>>,
): string {
  const prefix = `file://${tempDir()}`;
  for (const [product, log] of Object.entries(changelogs)) {
    const filePath = changelogUrl(prefix, product as Product).slice(
      "file://".length,
    );
