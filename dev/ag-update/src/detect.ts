 ...
 *  detectWords matches from the source-file search. This stage is dumb: any match — even in a
...
function classifyCandidate(
  ...
  if (change.type === "dependency") {
    ...
    const applies =
      ...
      dependency.frameworks.includes(
        change.dependency as (typeof dependency.frameworks)[number],
      );
    return applies ? "included" : "excluded";
  }
  if (
    change.framework !== null &&
...
/** Runs one source-file search over the combined detectWords of all candidates and returns the
 *  candidates that matched, with their occurrences filled in. */
function matchDetectWords(
  projectPath: string,
  candidates: DetectedChange[],
  sourceGlob: string[],
): DetectedChange[] {
  const words = new Set(
  ...
  const occurrencesByWord = new Map(
    [...words].map((word) => [word, [] as { file: string; line: number }[]]),
  );
...
export function containsWholeWord(content: string, word: string): boolean {
  for (
    let from = content.indexOf(word);
    from !== -1;
    from = content.indexOf(word, from + 1)
  ) {
