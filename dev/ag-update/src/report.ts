...
export function renderReport(
  ...
  const sections = [
    ...
    "# Required changes",
    ...(required.length > 0
      ? renderGroupedChanges(result, required)
      : ["No required changes were detected."]),
    "# Optional changes",
    ...(optional.length > 0
...
function renderGroupedChanges(
  ...
  for (const { product } of result.dependencies) {
    ...
    const majors = [
      ...new Set(
        productChanges.map((change) => majorOf(changeVersion(change.change))),
      ),
    ];
    majors.sort((a, b) => a - b);
    for (const major of majors) {
      ...
      const inTransition = productChanges
        ...
        .sort((a, b) =>
          compareVersions(changeVersion(a.change), changeVersion(b.change)),
        );
